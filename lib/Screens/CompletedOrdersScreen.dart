...
class CompletedOrdersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
          stream: _firebaseService.getCompletedOrders(),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return const Center(child: Text('No Completed orders available',style: TextStyle(color: blackColor,fontFamily: 'LexendRegular',fontSize: 20),));
            } else {
              List<QueryDocumentSnapshot> orders = snapshot.data!.docs.toList();
              // Sort orders by timestamp in ascending order
              orders.sort((a, b) => (b['CreatedAt'] as Timestamp)
