...
class InstallUninstallScreen extends StatefulWidget {
  final String title;
  final String docId;
  final String img;
  InstallUninstallScreen(
      {super.key, required this.docId, required this.img, required this.title});

  @override
  State<InstallUninstallScreen> createState() => _InstallUninstallScreenState();
}

class _InstallUninstallScreenState extends State<InstallUninstallScreen> {
  ...
  Future<String?> _uploadImage() async {
    ...
    try {
      final firebase_storage.Reference ref = firebase_storage
          ...
          .ref()
          .child('Images/Services/Install Uninstall')
                ...
                children: [
                  ...
                  widget.docId.isNotEmpty
                      ? ServicesListItemsandAddItems(
                          ...
                          categoryName: 'InstallUninstall',
                          screen: CommonServicesScreen(
                              categoryId: widget.docId,
                              servicename: 'InstallUninstall'),
                          ServiceQuerySnapshot:
                              FirebaseService().getInstallUninstall(),
                        )
                      : const SizedBox(),
                  const SizedBox(
                    height: 30,
