...
static Future<void> shareservicepartner(
    String orderid, BuildContext context, bool orderiscompleted) async {
              ...
              content: SingleChildScrollView(
                child: Column(
                  children: partners.map((partner) {
                    ...
                    String partnerId = partner.id; // Unique ID of the partner document
                    return StatefulBuilder(
                      builder: (context, setState) {
                        return CheckboxListTile(
                          title: Text(partnerName),
                          value: selectedPartners.any((element) => element['id'] == partnerId),
                          onChanged: (bool? value) {
                            setState(() {
                              if (value ?? false) {
                                selectedPartners.add({'id': partnerId, 'name': partnerName});
                              } else {
                                selectedPartners.removeWhere((element) => element['id'] == partnerId);
                              }
                            });
                          },
                        );
              ...
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                    ...
                    onDutypartner(orderid, context, orderiscompleted);
                  },
                  child: const Text(
                    'View OnDuty Partner',
                ...
                TextButton(
                  onPressed: () async {
                    await shareOrderDetailsWithPartners(orderid, selectedPartners);
...
static Future<void> shareOrderDetailsWithPartners(String orderid, List<Map<String, String>> selectedPartners) async {
  ...
  static Future<void> onDutypartner(
      String orderid, BuildContext context,bool isordercompleted) async {
    await showDialog<void>(
      context: context,
      builder: (BuildContext context) {
        return FutureBuilder<QuerySnapshot>(
              ...
              return AlertDialog(
                ...
                actions: <Widget>[
                  ...
                  TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
                      shareservicepartner(orderid, context,isordercompleted); // Close the dialog
                    },
